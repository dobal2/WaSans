...
public class CharacterController2D : MonoBehaviour
{
	...
	public int life = 10; //Life of the player
	...
	[SerializeField] private float afterImageSpacing = 0.1f;
	...
	public void Move(float move, bool jump, bool dash)
	{
		if (canMove) {
			...
			if (isDashing)
			{
				...
				if (Mathf.Abs(transform.position.x - lastAfterImageXpos) >= afterImageSpacing)
				{
					SpawnAfterImage();
				}

				//StartCoroutine("RestInvincible");
	...
	private void SpawnAfterImage()
	{
		...
		DashAfterImage afterImage = clone.GetComponent<DashAfterImage>();
		afterImage.Setup(playerSprite.sprite, transform.position, transform.localScale, playerSprite.flipX);
	...
	public void Restart()
	{
		...
		GameManager.Instance.SpawnEnemiesForStage(GameManager.Instance.currentStage);
	}
